...
$fileContent = file("leaderboard.txt", FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
...
foreach ($fileContent as $line) {
    // Split the line into code and value using the comma as a delimiter
    list($n, $s) = explode(",", $line);

    // Remove the semicolon at the end of the value and convert it to an integer
    $s = intval(rtrim($s, ';'));

    // Store the data in the array
    $data[] = ['username' => $n, 'score' => $s];
}

// Sort the array in descending order based on the 'value' field
rsort($data);

// Display the sorted data in the specified format
foreach ($data as $item) {
    echo $item['username'] . ' : ' . $item['score'] . "<br>";
}
